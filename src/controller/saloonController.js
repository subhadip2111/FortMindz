...
exports.getSaloonsByBeautician = async (req, res) => {
    try {
        const saloons = await Saloon.find({ beautician: req.params.beauticianId }) .populate('services') // Populate services array
        .exec();;
        res.json(saloons);
    } catch (error) {
        console.error(error);
...
exports.getAllSaloons = async (req, res) => {
    try {
        const saloons = await Saloon.find().populate('beautician').populate('services').exec();
...
exports.getSaloonById = async (req, res) => {
    try {
        const saloon = await Saloon.findById(req.params.saloonId).populate('services');
        if (!saloon) {
